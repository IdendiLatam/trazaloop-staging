-- 0089_textile_technical_passport_snapshot_fixes.sql
-- Trazaloop · Sprint T9B.1 (Textil) · Corrección funcional de la generación
-- del snapshot del pasaporte técnico textil (redefine la RPC de 0088).
--
-- Corrige seis problemas de T9B (0088), sin ampliar alcance y sin UI:
--   1. COMPOSICIÓN (grave): 0088 sumaba filter (where component_scope =
--      'main'), pero 'main' NO es un component_scope válido (los reales son
--      whole_product / main_fabric / secondary_fabric / lining / thread /
--      trim / other). Eso daba total=0 y estado 'incomplete' SIEMPRE. Ahora
--      la completitud se evalúa POR ALCANCE con la misma regla del dominio y
--      de la circularidad (0080): todos los alcances con datos deben sumar
--      100 ± 0,5; >100,5 es 'needs_review'; sin filas, 'not_started'.
--   2. RECOMENDACIONES: 0088 empujaba strings sueltos a recommendations_json.
--      Ahora son objetos estructurados { code, section_key, message }.
--   3. EVIDENCIAS: la completitud consideraba 'documented' con una sola
--      evidencia aceptada. Ahora distingue mejor (documented solo si hay
--      soporte aceptado y ninguna vinculada rechazada/vencida sin sustituto;
--      partially en el resto) y expone el desglose real.
--   4. PROVEEDORES: jsonb_agg(distinct jsonb_build_object(...)) es frágil;
--      se materializa la lista distinta antes de agregar.
--   5. CUIDADO/FIN DE VIDA: además de componentes reemplazables, refleja la
--      separabilidad evaluada (dato real disponible).
--   6. RESUMEN: la sección gaps_and_warnings ahora incluye el conteo real por
--      severidad y el resumen ejecutivo deriva el nivel de preparación de esas
--      cifras (antes quedaban en 'documented' fijo).
--
...
-- funciones. Solo lectura de los módulos existentes + escritura de la fila
-- del pasaporte bajo el flag interno (respeta el trigger de 0085). Sin UI,
-- rutas, impresión, QR, PDF, IA, ACV, huella. CERO cambios CPR.
...
begin
  ...
  if v_comp_status = 'not_started' then
    ...
    v_gaps := v_gaps || jsonb_build_object(
      'gap_code', 'PAS-COMP-002', 'severity', 'warning', 'section_key', 'fiber_composition',
    ...
    v_recs := v_recs || jsonb_build_object(
      ...
      'priority', 'medium', 'related_gap_code', 'PAS-COMP-002');
  ...
  -- =========================================================================
  -- 5.7 Evidencias (fix #3: completitud más honesta; estados reales)
  -- =========================================================================
  with linked as (
    select distinct e.id, e.title, e.evidence_type, e.status, e.valid_until, el.link_type
  ...
  -- =========================================================================
  -- 5.9 Circularidad (fix #2: auto-selección de la última completed)
  -- =========================================================================
  -- Si el pasaporte no fijó una evaluación, se busca la 'completed' más
  -- reciente de la referencia; si no hay completed pero sí draft/in_review,
  -- se advierte; si no hay ninguna, se genera brecha.
  ...
  v_snapshot := jsonb_build_object(
    ...
    'passport', jsonb_build_object(
      ...
      'circularity_assessment_auto_selected', (v_assessment is not null and v_sec_circularity->>'assessment_code' is not null)),
