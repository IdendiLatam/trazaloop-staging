-- 0090_textile_technical_passport_snapshot_sources_closure.sql
-- Trazaloop · Sprint T9B.2 (Textil) · Cierre de fuentes, evidencias y warnings
-- del snapshot del pasaporte técnico textil (redefine la RPC de 0089).
--
-- Cierra cinco puntos pendientes de T9B.1, sin ampliar alcance y sin UI:
--   1. snapshot_json.evidences.items ahora incluye evidencias vinculadas a
--      TODAS las entidades del pasaporte, incluidas production_order,
--      order_process_step, circularity_assessment y technical_passport (antes
--      solo hasta output_lot). T9C construirá la UI desde snapshot_json, por lo
--      que las evidencias visibles deben estar completas ahí. Cada ítem lleva
--      metadata (evidence_id, title, evidence_type, status, entity_type,
--      entity_id, link_type, document_date, valid_until, file_name, created_at,
--      updated_at) — nunca signed URLs ni file_path.
--   2. data_sources_json.source_records.evidence_links: colección explícita de
--      los vínculos (id, evidence_id, entity_type, entity_id, link_type,
--      created_at) para que el source_hash detecte cambios de vínculo
--      (relink, cambio de link_type, desvinculación). NOTA: textile_evidence_links
--      no tiene columnas updated_at ni status (solo created_at); se registran
--      las columnas reales y el conjunto de IDs captura altas/bajas.
--   3. data_sources_json.source_records.process_steps: colección explícita de
--      los pasos de proceso de la orden (id, order_id, step_type, process_id,
--      outsourced_process_id, status, planned_date, completed_date, created_at,
--      updated_at) para que el hash detecte cambios de ruta. NOTA: el modelo usa
--      step_type (no process_type) y planned_date/completed_date (no started_at/
--      finished_at); se usan los nombres reales.
--   4. Warning PAS-TRACE-005 cuando hay lote y orden pero la orden no tiene
--      pasos de proceso documentados (no bloquea).
--   5. Composición no documentada se normaliza a PAS-COMP-002 (antes
--      PAS-COMP-003). El caso 'excede 100%' (needs_review) pasa a PAS-COMP-003;
--      PAS-COMP-001 (no suma 100 por alcance) se mantiene.
--
...
-- rutas, impresión, QR, PDF, IA, ACV, huella. CERO cambios CPR. Nombres de
-- entity_type verificados contra 0075/0078/0084 (no se inventan).
