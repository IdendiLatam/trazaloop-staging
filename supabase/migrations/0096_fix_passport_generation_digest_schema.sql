-- ============================================================================
-- Trazaloop · Sprint T9E.1 (Textil) · Fix digest() en la GENERACIÓN del
-- pasaporte técnico (misma clase de defecto que 0095)
-- ============================================================================
--
-- DEFECTO (encontrado por la prueba MANUAL REAL de T9E.1): las funciones
-- generate_textile_technical_passport_base (versión efectiva: 0086) y
-- generate_textile_technical_passport_full_snapshot (versión efectiva:
-- 0091) calculan source_hash con digest() SIN calificar mientras fijan
-- `set search_path = public`. pgcrypto vive en `extensions`, así que TODA
-- generación de snapshot fallaba en ejecución (notice=generation_failed)
-- desde su origen — los tests estáticos jamás lo detectaron.
--
-- CORRECCIÓN: se recrean AMBAS funciones EXACTAMENTE como su última
-- versión aplicada (0086 y 0091 respectivamente) con una única diferencia:
-- `extensions.digest(...)`. search_path sigue fijado a `public`.
-- 0084–0091 NO se modifican retroactivamente.
--
-- ROLLBACK: re-ejecutar las definiciones de 0086 y 0091 (restaura el
-- defecto) — documentado solo por completitud.
-- ============================================================================

-- --- generate_textile_technical_passport_base (0086 + fix digest) ---
...
begin
  ...
  v_hash := encode(extensions.digest(v_sources::text, 'sha256'), 'hex');
...
-- --- generate_textile_technical_passport_full_snapshot (0091 + fix digest) ---
