...
async function deleteCatalogRecord(
  ...
  if (error) {
    ...
    return { error: `No fue posible eliminar ${label} (verifica tu rol en la empresa).` };
  }
  if (!data || data.length === 0) {
    return { error: `${capitalize(label)} no existe o no pertenece a tu empresa.` };
  }
...
export async function updateTextileCustomFiberAction(
  ...
  if (error || !data || data.length === 0) {
    return { error: "La fibra no existe, es del catálogo base o no pertenece a tu empresa." };
  }
  revalidatePath(`${CATALOGS_PATH}/fibers`);
  revalidatePath(CATALOGS_PATH);
  return { error: null };
...
export async function deleteTextileCustomFiberAction(
  ...
  if (!data || data.length === 0) {
    return {
      error: "La fibra no existe, es del catálogo base o no pertenece a tu empresa.",
    };
  }
  revalidatePath(`${CATALOGS_PATH}/fibers`);
  revalidatePath(CATALOGS_PATH);
