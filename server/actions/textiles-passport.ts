...
export async function generateTextilePassportSnapshotAction(
  ...
  if (!passport) return { error: "El pasaporte no existe o no pertenece a tu empresa." };
...
export async function createTextilePassportDraftAction(input: {
  ...
  if (!reference) return { error: "La referencia no existe o no pertenece a tu empresa." };
  ...
  if (outputLotId) {
    ...
    if (!lotRef) return { error: "El lote producido/final no existe o no pertenece a tu empresa." };
  ...
  if (circularityAssessmentId) {
    ...
    if (!assessmentRef) {
      return { error: "La evaluación de circularidad no existe o no pertenece a tu empresa." };
    }
