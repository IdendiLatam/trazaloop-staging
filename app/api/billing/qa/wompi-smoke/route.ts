...
export async function POST(request: Request) {
  ...
  if (accion === "retire_subscription") {
    // Retiro ADMINISTRATIVO por la primitiva canónica. Aquí no hay ni un
    // `update` a mano: si la función se niega, se anota la negativa.
    const { data, error } = await admin.rpc("billing_retire_subscription", {
      p_subscription_id: String(cuerpo.subscription_id ?? ""),
      p_reason_code: String(cuerpo.reason_code ?? ""),
      p_reason: String(cuerpo.reason ?? ""),
    });
    if (error) return no(`RETIRE_FAILED:${error.message}`, 500);
